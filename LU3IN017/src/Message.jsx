...
function Message(props) {
    ...
    useEffect(() => {
        const getAuthor = async () => {
            try {
                let response = await serverConfig(axios.get, `/api/user/${props.messageInfo.uid}`);
    ...
    const getReplies = async () => {
        try {
            let response = await serverConfig(axios.get, `/api/reply?mid=${props.messageInfo.mid}`);
    ...
    return (
        <div className="message-container">
            ...
            <div className="message-info">
                Publié le {formattedDate} par <a onClick={() => props.toUserPage(author)}>{author ? author.uname : ""}</a>
            </div>
            ...
            <div className="message-icons">
                ...
                <div className="message-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" className="message-icon bi bi-reply-fill" viewBox="0 0 16 16">
                ...
                {props.messageInfo.uid === props.currentUser.uid && (
                    <div className="message-icon">
                        <button type="button" className="btn-icon" onClick={() => props.removeMessage(props.messageInfo.mid)}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" className="message-icon bi bi-trash3" viewBox="0 0 16 16">
            ...
            <form className="new-reply">
                <div>
                    ...
                    <textarea className="reply-content" id={`reply-${props.messageInfo.mid}`} placeholder="Veuillez saisir votre message de réponse"></textarea>
                </div>
